...
Template.days.helpers({
	...
	allEvents: function () {
		...
		if(Roles.userIsInRole(Meteor.user()._id, 'admin'))
			return EventList.find({}, {sort: {start: {dateTime: 1}}}).fetch().reverse();
		else
  			return EventList.find({$where: "(this.users.length < this.limit) || (Meteor.user().events.indexOf(this._id) > -1) "}, {sort: {start: {dateTime: 1}}}).fetch().reverse();
	},

	checkDay: function (a) {
		
		// console.log(this.summary + ' ' + Session.get("previousDay") + ' vs ' + moment(this.start.dateTime).format('DDD'));
		var day1 = Session.get("previousDay");
		var day2 = moment(a).format('DDD');
		if(day1===day2) {
			return false;
		}
		else{
			Session.set("previousDay", day2);
			console.log("GO!");
			return true;
		}
	},
...
Template.days.events({
  "click #make-decision": function (event) {
    Meteor.call("userDecide",Meteor.userId());
  },
  
  "click #userClear-button": function() {
    Meteor.call("userClear");
...
Template.oneEvent.events({
  "click #register": function(event) {
    ...
    Session.set("Q",false);
    Session.set("T",false);
    Session.set("P_add",false);
    Session.set("P_limit",false);
    Session.set("F_limit",false);
    Session.set("F_add", false);
    Session.set("L_add", false);
    
    console.log($(q.target).text());
    

    Meteor.call("eventSet",this._id, Meteor.userId());
    // if(Session.get("Q")){
    //   if($(q.target).text() == "Register"){
    //      $(q.target).text('Unregister');
    //     // Materialize.toast("You've registered for this event!" , 1000);
    //   }
    //   else{
    //     $(q.target).text('Register');
    //   Materialize.toast("You've unregistered for this event!" , 1000);
    //   }
    //   // $(event.target).parent().parent().prev().toggleClass("chosen");
    // }
      var s = '<i class="material-icons left">grade</i>Register';
  
    
    if(Session.get("P_limit"))
      Materialize.toast("Sorry, you have used your limit of <strong> 	&nbsp;"+ ' '+RegisterLimit.find().fetch()[0].payable+"&nbsp; payable events</strong>. Please uncheck some of them to register for this one" , 4000);
    if(Session.get("L_add"))
      Materialize.toast("You've registered for a not limited event " , 4000);
    if(Session.get("P_add"))
      Materialize.toast("You've registered for <strong> &nbsp; "+Meteor.user().payable+" &nbsp; of &nbsp; "+RegisterLimit.find().fetch()[0].payable+" &nbsp; payable events </strong> &nbsp; avaible for you. Please remember that you will have to pay on 18th of January in Nymble." , 4000);
    if(Session.get("F_add"))
      Materialize.toast("You've registered for <strong> &nbsp; "+Meteor.user().free+" &nbsp; of &nbsp; "+RegisterLimit.find().fetch()[0].free+" &nbsp; free events </strong> &nbsp; avaible for you." , 4000);
    if(Session.get("F_limit"))
      Materialize.toast("Sorry, you have used your limit of <strong> &nbsp;"+ ' '+RegisterLimit.find().fetch()[0].free+" &nbsp; free events</strong>. &nbsp; Please uncheck some of them to register for this one" , 4000);
  }
});
...
Template.oneEvent.helpers({
  ...
  isEventFull: function() { 
    console.log('isEventFull'+this);
    if(!Role.users.IsInRole(Meteor.user()._id, 'admin') && !Meteor.user().decided && this.users.length >= this.limit)
      return false;
    else
      return true;
  },

  isEventChosen: function() {
  	return (Meteor.user().events.indexOf(this._id) > -1);
  }
})
...
Template.adminEvent.events({
  // "submit #type": function (event) {
  //   event.preventDefault();
  //   Meteor.call("eventUpdate",this._id, event.target.id, event.target.text.value);
  //   event.target.text.value = "";
  // },

  "submit #manually-add": function (event) {
    event.preventDefault();
    Meteor.call("userManuallyAdd",event.target.text.value, this);
    event.target.text.value = "";
  },
  
  "click #free": function (event) {
    // event.preventDefault();
    Meteor.call("eventUpdate",this._id, 'eventType', 'free');
    // event.target.text.value = "";
  },
  
  "click #payable": function (event) {
    // event.preventDefault();
    Meteor.call("eventUpdate",this._id, 'eventType', 'payable');
    // event.target.text.value = "";
  },
  
  "click #notLimited": function (event) {
    // event.preventDefault();
    Meteor.call("eventUpdate",this._id, 'eventType', 'notLimited');
    // event.target.text.value = "";
  },
  ...
  "submit #summary": function (event) {
    event.preventDefault();
    Meteor.call("eventUpdate",this._id, event.target.id, event.target.text.value);
    event.target.text.value = "";
  },
  
  "submit #limit": function (event) {
    event.preventDefault();
    Meteor.call("eventUpdate",this._id, event.target.id, event.target.text.value);
    event.target.text.value = "";
  },
  
  "submit #start-date": function (event) {
    E=event;
    console.log(E);
    event.preventDefault();
    Meteor.call("eventUpdateDateStart",this._id, event.target.text.value);
    event.target.text.value = "";
  },
  
  "submit #end-date": function (event) {
    event.preventDefault();
    Meteor.call("eventUpdateDateEnd",this._id, event.target.text.value);
    event.target.text.value = "";
  },
  
  "click #event-remove": function() {
    console.log("trying to remove " + this._id);
    Meteor.call("eventRemove", this._id);
  },
...
// 		// $('.days').append('<div><div><ul>');
// 		// $('.day_card').prepend('</ul></div></div><div class="card day"><div class="card-content">');
// 		// $('.day_card').append('<ul>');

// 		$('.day_card').each(function(index) {
...
// 			s='<div class="card day"><div class="card-content">';
// 			$(this).before(s);
// 			s='<ul>';
// 			$(this).after(s);
// 		});

		// $('#'+a).prepend('</ul></div></div><div class="card day"><div class="card-content">');
		// $('#'+a).append('<ul>');

		// console.log('inserted for ' +'#'+a);
		// }
// });


Template.eventsDownload.events({
	"click #authorize-button": function (event) {
		...
		$("#authorize-button").html('Please try again');
		
		var request = gapi.client.calendar.events.list({
	      'calendarId': 'bg14ap8mqj9qamumgac9lno1no@group.calendar.google.com',
